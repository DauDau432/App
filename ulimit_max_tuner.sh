...
EOF
    ...
    # Áp dụng cấu hình và bỏ qua lỗi cho các tham số không hỗ trợ
    echo -e "${YELLOW}[*] Đang áp dụng cấu hình sysctl...${RESET}"
    sysctl_output=$(sysctl -p 2>&1)
    sysctl_exit_code=$?
    
    # Đếm số lỗi và cảnh báo
    error_count=$(echo "$sysctl_output" | grep -i "error\|unknown\|invalid\|deprecated" | wc -l)
    success_count=$(echo "$sysctl_output" | grep -c "=" || echo "0")
    
    # Loại bỏ các tham số không hợp lệ từ file
    if [ $error_count -gt 0 ]; then
        echo -e "${YELLOW}[*] Phát hiện một số tham số không được hỗ trợ, đang làm sạch...${RESET}"
        
        # Xóa các dòng có tham số không hợp lệ
        ...
        # Kiểm tra và xóa tcp_ecn nếu không hỗ trợ
        if echo "$sysctl_output" | grep -q "tcp_ecn.*invalid\|tcp_ecn.*unknown"; then
            sed -i '/net.ipv4.tcp_ecn/d' /etc/sysctl.conf
        fi
        
        # Áp dụng lại sau khi làm sạch
        sysctl -p > /dev/null 2>&1
    fi
    
    if [ $sysctl_exit_code -eq 0 ] || [ $success_count -gt 0 ]; then
        echo -e "${GREEN}[+] Đã cấu hình sysctl tối ưu (${success_count} tham số áp dụng thành công).${RESET}"
        if [ $error_count -gt 0 ]; then
            echo -e "${YELLOW}[*] Đã bỏ qua ${error_count} tham số không được hỗ trợ trên hệ thống này.${RESET}"
        fi
        ...
        echo -e "${RED}[-] Có lỗi khi áp dụng sysctl.${RESET}"
        echo -e "${YELLOW}[*] Chi tiết lỗi:${RESET}"
        echo "$sysctl_output" | grep -i "error\|unknown\|invalid" | head -5
...
echo "  [4] Khôi phục mặc định hệ thống"
