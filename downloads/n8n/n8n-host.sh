...
import_data() {
    ...
    if ! sudo docker exec -u node "${N8N_CONTAINER_NAME}" ${import_cmd} > "${import_log}" 2>&1; then
        ...
        echo -e "2. Tim workflow ${GREEN}[H2cloud] Import Workflows, Credentials${NC} trong danh sach 'Workflows'."
...
show_help() {
    echo "N8N Cloud Manager - Cong cu quan ly N8N tren H2cloud"
...
show_menu() {
  ...
  printf "${CYAN}|                    Powered by H2cloud - https://h2cloud.vn                    |${NC}\n"
  printf "${CYAN}+==================================================================================+${NC}\n"
  echo ""
  echo -e " ${YELLOW}Phim tat: Nhan Ctrl + C hoac nhap 0 de thoat${NC}" 
  echo -e " ${GREEN}Xem huong dan:${NC} ${CYAN}https://h2cloud.vn/link/n8n-cloud-docs${NC}"
  echo "------------------------------------------------------------------------------------"
